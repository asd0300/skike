...
mydatabase = conn['test'] 
# Access collection of the database 
mycollection=mydatabase['t222_sort']
...
def gettotal(locLN1,locLN2,dateTodate,count=0):
    ...
    'apollographql-client-version': 'v93_10_3_ad_8254b7ab069'
    }
...
# getHotel(str(param.locLN['tokyo'][0]),str(param.locLN['tokyo'][1]), "20211024/20211025")
# print(param.locLN['tokyo'][0],param.locLN['tokyo'][1])
start_date = date(2021, 10, 21)
end_date = date(2021, 10, 22)
start_time_total = time.time()
for single_date in daterange(start_date, end_date):
    next1 = single_date + timedelta(days=1)
    a = next1.strftime("%Y%m%d")
    b = single_date.strftime("%Y%m%d")
    ...
    print('------finish-------'+single_date.strftime("%Y-%m-%d"))
end_time_total = time.time()
print("--- %s total seconds ---" % (end_time_total - start_time_total))
