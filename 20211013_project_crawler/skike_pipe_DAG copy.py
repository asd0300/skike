...
def get_hotel_to_mongo():
    locLN_KR = {'首爾': ["126.977966/37.566536", "81393"],
    ...
    conn = MongoClient("mongodb://skike4:{}@ec2-18-191-175-148.us-east-2.\
        compute.amazonaws.com:27017/?authSource=admin&readPreference=primary&\
        appname=MongoDB%20Compass&directConnection=true&ssl=false".format(config.MONGO_PASS_SKIKE_EC22))
    ...
    def gettotal(locLN1, locLN2, dateTodate, count=0):
        ...
        param = '''{{"dt":1,"iph":1,"tz":-480,"pra":"","channel":"b,isd:0,sps:22",\
            ...
            .format(locLN1, locLN2, dateTodate, count)
        # print(1)
        # print(param)
        param.replace('"', '\"')
        ...
        while (total > count):
            try:
                ...
                param = '''{{"dt":1,"iph":1,"tz":-480,"pra":"","channel":"b,isd:\
                    ...
                    "acclim":25}}'''.format(locLN1, locLN2, dateTodate, count)
                param.replace('"', '\"')
    ...
    def getJPhotelSigleTh(date):
        ...
        for items in locLN_KR:
            getHotel(str(locLN_KR[items][0]),
                     str(locLN_KR[items][1]),
                     "{}/{}".format(b, a))
    ...
    def getJPhotelSigleThNoSave(date):
        ...
        b = date.strftime("%Y%m%d")
        for items in locLN_KR:
            getHotelNoSave(str(locLN_KR[items][0]),
                           str(locLN_KR[items][1]),
                           "{}/{}".format(b, a))
            print("ok, {} {}".format(items, b))
            time.sleep(random.randint(1, 5))
    ...
    for i in range(1, 19):
        ...
        datelist = [single_date for single_date in daterange(start_date,
                                                             end_date)]
        ...
        datelistNoSave = [single_date for single_date in daterange(start_date,
                                                                   end_date)]
        start_time = time.time()
        with concurrent.futures.ThreadPoolExecutor(max_workers=5) as executor:
...
def insert_hotel_mongo_to_RDS():
    ...
    conn = MongoClient("mongodb://skike4:{}@ec2-18-191-175-148.us-east-2.compute.\
        amazonaws.com:27017/?authSource=admin&readPreference=primary&appname=MongoDB%\
    ...
    print(yesterday)
    mycollection = mydatabase['({})skike_hotel_KR\
        '.format(str(date.today() + timedelta(days=1)))]
    pipe_total = [
                 {
                     '$project': {
                         'priceHistogram': 0,
    ...
    for num in range(0, mongo_result_count, 50):
        ...
        for i in results:
            ...
            try:
                ...
                rdsDB.commit()
                a += 1
                print("now in "+str(a)+"total 50 ,mongo in "+str(num)
...
def get_flight_ticket_mongo():
    conn = MongoClient("mongodb://skike4:{}@ec2-18-191-175-148.us-east-2.compute\
                       .amazonaws.com:27017/?authSource=admin&readPreference=primary&appname=\
                       MongoDB%20Compass&directConnection=true&ssl=false"
                       .format(config.MONGO_PASS_SKIKE_UBUNTU))
    mydatabase = conn['skike']
    ...
    def test(position, date1):
        ...
        try:
            ...
            print("ERROR:", url, e)
    cityFList_KR = ['SEL', 'PUS', 'CJU', 'TAE', 'USN', 'KPO', 'HIN', 'RSU',
                    'YNY', 'WJU', 'KUV', 'KWJ', 'MWX']
    ...
    for i in range(1, 4):
        ...
        datelist = [single_date.strftime("%Y-%m-%d") for single_date
    ...
    mydatabase = conn['skike']
    mycollection = mydatabase['({})_ticket_to_Taiwan_from_KR'
                              .format(str(date.today()+timedelta(days=1)))]
...
def flight_data_insert():
    ...
    conn = MongoClient("mongodb://skike4:{}@ec2-18-191-175-148.us-east-2.compute.\
                       ...
                       .format(config.MONGO_PASS_SKIKE_UBUNTU))
    mydatabase = conn['skike']
    mycollection = mydatabase['({})skike_ticket_to_KR'
                              .format(str(date.today()+timedelta(days=1)))]
    pipetest = [
               {
                   '$match': {
        ...
        for product in i['productInfoList']:
            ...
            for item in flight_info_list:
                try:
                    ...
                    shoppingId = product['policyInfoList'][0]['productKeyInfo']
                    ['shoppingId']
                except Exception as e:
                    print("Exeception occured:{}".format(e))
            ...
            for item in flight_info_list:
                ...
                try:
                    cursor.execute(sql_stopover, (terminal_arrive_time,
                                   ...
                                   flight_arrive_terminal, shoppingId))
                    rdsDB.commit()
                except Exception as e:
                    print("Exeception occured:{}".format(e))
            ...
            if product['policyInfoList'][0]['priceDetailInfo']['adult'] \
                ...
                for policy_item in policy_info_list:
                    ...
                    a = "remarkTokenKey=" + remark_token_key + "&" +\
                        "criteriaToken=" + criteriaToken + "&" +\
                        "shoppingId=" + shoppingId + "&" + "groupKey=" +\
                        groupKey
                    url += a
            ...
            elif product['policyInfoList'][0]['priceDetailInfo']['child'] is not None:
                for policy_item in policy_info_list:
