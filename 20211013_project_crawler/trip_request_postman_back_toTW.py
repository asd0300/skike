...
def test(strat_position,arrive_position,date):
    try:
        ...
        print("OK------------------position "+str(strat_position)+" to "+str(arrive_position)+" date "+date)
        # print(response)
...
def dateCrawlerBKTW():
    ...
    for single_date in daterange(start_date, end_date):
        ...
        for item in cityFList:   
            test(item,'TPE', single_date.strftime("%Y-%m-%d"))
            time.sleep(0.3)
